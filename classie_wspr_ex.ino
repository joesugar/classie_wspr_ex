...
/* Transmit frequency shift can be calculated as
 * (BIT_PERIOD_DENOM / BIT_PERIOD_NUMBER) or 1.46 Hz.
 * As an integer a value of 2 works.
 */
...
void setup()
{
    ...
    {
        transmitState = WAIT_FOR_CALIBRATE_COMPLETE;
        first_test_time = TRUE;
    }
    else
    {
...
void loop()
{
    ...
    if (WAIT_FOR_CALIBRATE_COMPLETE == transmitState)
    {
        ...
        if (calibrate == LOW)
        {
            ...
            if (first_test_time)
            {           
                /* First time in the state so set the correction
                 * assigned for calibration and set the oscillator 
                 * to the calibration frequency.
                 */
                si5351.set_correction(CALIBRATE_CORRECTION);
